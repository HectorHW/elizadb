...
use crate::{storage::{Database, Key, IndexLocation}, doublemap::DoubleMap};

impl<const SMALLSIZE: usize> Database<SMALLSIZE> {
    pub fn new(terms: HashMap<String, u8>, small_keys: Vec<Key>, small_storage: Vec<u8>, big_storage: HashMap<Key, HashSet<u8>>) -> Self {
    ...
    fn build_index(small_keys: &[Key], big_storage: &HashMap<Key, HashSet<u8>>) -> HashMap<Key, IndexLocation> {
